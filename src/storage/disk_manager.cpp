...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    off_t offset_file = static_cast<off_t>(page_no) * PAGE_SIZE;//字节偏移量
    if(lseek(fd, offset_file, SEEK_SET)==-1) {
        throw InternalError("DiskManager::write_page Error: lseek Error");
    }
    ssize_t bytes = write(fd, offset, num_bytes);
    if( bytes != num_bytes) {
        throw InternalError("DiskManager::write_page Error: write Error");
    }
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    off_t offset_file = static_cast<off_t>(page_no) * PAGE_SIZE;//字节偏移量
    if(lseek(fd, offset_file, SEEK_SET)==-1) {
        throw InternalError("DiskManager::read_page Error: lseek Error");
    }
    ssize_t bytes = read(fd, offset, num_bytes);
    if( bytes != num_bytes) {
        throw InternalError("DiskManager::read_page Error: read Error");
    }
